package application;

import java.text.ParseException;
import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.TimeZone;

public class Program {

	public static void main(String[] args) throws ParseException {
		// TODO Auto-generated method stub
		
		Date x1 = new Date();
		Date x2 = new Date(System.currentTimeMillis());
		SimpleDateFormat sdf1 = new SimpleDateFormat("dd/MM/yyy");
		SimpleDateFormat sdf2 = new SimpleDateFormat("dd/MM/yyy HH:mm:ss");
		SimpleDateFormat sdf3 = new SimpleDateFormat("dd/MM/yyy HH:mm:ss");
		sdf3.setTimeZone(TimeZone.getTimeZone("GMT"));
		
		Date y1 = sdf1.parse("30/03/2020");
		Date y2 = sdf2.parse("30/03/2020 11:10:35");
		
		System.out.println(y1);
		System.out.println(y2);
		
		System.out.println(sdf1.format(y1));
		System.out.println(sdf2.format(y2));
		System.out.println("Data e hora atual x1: " + sdf2.format(x1));
		System.out.println("Data e hora atual x2: " + sdf2.format(x2));
		
		System.out.println("----------------------------------------");
		System.out.println("Data no padrão GMT");
		
		System.out.println(sdf3.format(y1));
		System.out.println(sdf3.format(y2));
		System.out.println("Data e hora atual x1: " + sdf3.format(x1));
		System.out.println("Data e hora atual x2: " + sdf3.format(x2));
		
		


	}

}
